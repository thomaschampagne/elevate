        ...
        this.activitiesSynchronize.sync(this.fastSync).then(
            ...
            (error: any) => {
                ...
                const errorUpdate: any = {
                    stravaId:
                        window.currentAthlete && window.currentAthlete.get("id")
                            ? window.currentAthlete.get("id")
                            : null,
            ...
            (progress: SyncNotifyModel) => {
                ...
                switch (progress.step) {
                    ...
                    case "updatingSyncDateTime":
                        ...
                        console.log(
                            "Sync time: " + Helper.secondsToHHMMSS(totalSec) + " (" + Math.round(totalSec) + "s)"
                        );
                        break;
                }

                $("#syncStep").html(
                    "Activity group <strong>" + progress.pageGroupId + "</strong> &#10141; " + stepMessage
                );
