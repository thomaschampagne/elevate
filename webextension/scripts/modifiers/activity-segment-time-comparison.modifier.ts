                ...
                $items.each(() => {
                    ...
                    $.getJSON(segmentEffortInfoUrl, (segmentEffortInfo: EffortInfo) => {
                        ...
                        const komSeconds: string = Helper.HHMMSStoSeconds(
                                (this.isFemale ? segmentEffortInfo.qom_time : segmentEffortInfo.kom_time).replace(
                                    /[^0-9:]/gi,
                                    ""
                                )
                            ).toString(),
                        ...
                        this.findCurrentSegmentEffortDate(segmentEffortInfo.segment_id, segmentEffortId).then(
                            (currentSegmentEffortDateTime: Date, leaderBoardData: EffortInfo[]) => {
            ...
            if (currentYearPRSeconds) {
                if (!previousPersonalSeconds) {
                    ...
                    if (previousBestResultThisYear) {
                        deltaTime =
                            currentActivityResult.elapsed_time_raw - previousBestResultThisYear.elapsed_time_raw;
                        percentTime =
                            (currentActivityResult.elapsed_time_raw / previousBestResultThisYear.elapsed_time_raw - 1) *
                            100;
                        deltaYearPRCell.html(
