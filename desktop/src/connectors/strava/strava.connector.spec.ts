...
describe("StravaConnector", () => {
    ...
    describe("Perform strava api calls", () => {
        ...
        it("should perform a successful strava api request", done => {
            ...
            const stravaTokensUpdaterSpy = spyOn(stravaConnector, "stravaTokensUpdater").and.returnValue(
                Promise.resolve()
            );
            ...
            promise.then(
                result => {
                    ...
                    expect(stravaTokensUpdaterSpy).toHaveBeenCalledWith(
                        syncEvents$,
                        stravaConnector.stravaConnectorInfo
                    );
    ...
    describe("Postpone next strava api calls depending on rate limits", () => {
        it("should compute next call wait time (0% of available calls under 15min)", done => {
            ...
            // When
            const result = StravaConnector.computeNextCallWaitTime(
                currentCallsCount,
                thresholdCount,
                timeIntervalSeconds
            );
        ...
        it("should throw when computing next call wait time with invalid params (negative numbers)", done => {
            ...
            const expectedErrorMessage =
                "Params must be numbers and positive while computing strava next call wait time";
        ...
        it("should get streams of an activity", done => {
            ...
            promise.then(
                (activityStreamsModel: ActivityStreamsModel) => {
                    ...
                    expect(activityStreamsModel.distance).toEqual(
                        _.find(fakeStreamsFixture, { type: "distance" }).data
                    );
                    expect(activityStreamsModel.latlng).toEqual(_.find(fakeStreamsFixture, { type: "latlng" }).data);
                    expect(activityStreamsModel.altitude).toEqual(
                        _.find(fakeStreamsFixture, { type: "altitude" }).data
                    );
                    ...
                    expect(activityStreamsModel.heartrate).toEqual(
                        _.find(fakeStreamsFixture, { type: "heartrate" }).data
                    );
                    ...
                    expect(activityStreamsModel.watts_calc).toEqual(
                        _.find(fakeStreamsFixture, { type: "watts_calc" }).data
                    );
                    expect(activityStreamsModel.grade_smooth).toEqual(
                        _.find(fakeStreamsFixture, { type: "grade_smooth" }).data
                    );
