...
export class StravaConnector extends BaseConnector {
    ...
    public static generateFetchBareActivitiesPageEndpoint(
        page: number,
        perPage: number,
    ...
    public static computeNextCallWaitTime(
        currentCallsCount: number,
        thresholdCount: number,
        timeIntervalSeconds: number
    ): number {
        const notNumbers =
            !_.isNumber(currentCallsCount) || !_.isNumber(thresholdCount) || !_.isNumber(timeIntervalSeconds);
                        ...
                        if (_.isEmpty(syncedActivityModels)) {
                            ...
                            return this.getStravaActivityStreams(<number>bareActivity.id).then(
                                (activityStreamsModel: ActivityStreamsModel) => {
                                    try {
                                        ...
                                        syncedActivityModel.start_timestamp =
                                            new Date(bareActivity.start_time).getTime() / 1000;
                                        ...
                                        syncedActivityModel.id =
                                            syncedActivityModel.id +
                                            "-" +
                                            BaseConnector.hashData(syncedActivityModel.start_time, 8);
                            ...
                            if (_.isArray(syncedActivityModels) && syncedActivityModels.length === 1) {
                                ...
                                if (this.stravaConnectorInfo.updateSyncedActivitiesNameAndType) {
                                    ...
                                    syncEvents$.next(
                                        new ActivitySyncEvent(ConnectorType.STRAVA, null, syncedActivityModel, false)
                                    );
                                }
                            } else {
    ...
    public appendPowerStream(
        ...
        if (!bareActivityModel.hasPowerMeter) {
            if (!_.isEmpty(activityStreamsModel.watts_calc)) {
                ...
                if (
                    bareActivityModel.type === ElevateSport.Ride ||
                    ...
                    try {
                        ...
                        logger.error(
                            "Unable to estimated power on activity started at: " + bareActivityModel.start_time,
                            err
                        );
    ...
    public stravaApiCall(syncEvents$: Subject<SyncEvent>, url: string, tries: number = 1): Promise<any> {
        ...
        return this.stravaTokensUpdater(syncEvents$, this.stravaConnectorInfo)
            .then(() => {
            ...
            .then((response: IHttpClientResponse) => {
                ...
                this.nextCallWaitTime = StravaConnector.computeNextCallWaitTime(
                    rateLimits.instant.usage,
                    rateLimits.instant.limit,
                    ...
                    case HttpCodes.TooManyRequests:
                        ...
                        if (maxTriesReached) {
                            ...
                            const errDesc = `Strava ${
                                isInstantQuotaReached
                                    ? "instant quota reached"
                                    : isDailyQuotaReached
                                    ? "daily quota reached"
                                    : ""
                            }, retry sync later.`;
                        ...
                        syncEvents$.next(
                            new GenericSyncEvent(ConnectorType.STRAVA, `Still processing... Please wait few minutes.`)
                        );
                        const logMessage = `${
                            isInstantQuotaReached
                                ? "Instant quota reached"
                                ...
                                ? "Daily quota reached"
                                : ""
                        }. Waiting ${retryInTime} before continue.`;
    ...
    public stravaTokensUpdater(
        ...
        const isAccessTokenValid =
            stravaConnectorInfo.accessToken && stravaConnectorInfo.expiresAt > this.getCurrentTime();

        if (!stravaConnectorInfo.accessToken || !stravaConnectorInfo.refreshToken) {
            authPromise = this.stravaAuthenticator.authorize(
            ...
            logger.debug(
                "Access token is still valid, we keep current access token, no authorize and no refresh token"
            );
