...
export class FileSystemConnector extends BaseConnector {
                        ...
                        return parseSportsLibActivity
                            .then((event: EventInterface) => {
                                ...
                                return event
                                    ...
                                    .reduce(
                                        (
                                            previousActivityProcessed: Promise<void>,
                                            sportsLibActivity: ActivityInterface
                                        ) => {
                                            return previousActivityProcessed.then(() => {
                                                // Is an transition activity (e.g. from swim to cycling for triathlon), then skip it
                                                if (
                                                    sportsLibActivity &&
                                                    sportsLibActivity.type === ActivityTypes.Transition
                                                ) {
                                                ...
                                                return this.findSyncedActivityModels(
                                                    ...
                                                    if (_.isEmpty(syncedActivityModels)) {
                                                        try {
                                                            ...
                                                            const bareActivity = this.createBareActivity(
                                                                sportsLibActivity
                                                            );
                                                            ...
                                                            const primitiveSourceData = this.extractPrimitiveSourceData(
                                                                sportsLibActivity
                                                            );
                                                            ...
                                                            syncedActivityModel.sourceConnectorType =
                                                                ConnectorType.FILE_SYSTEM;
                                                        ...
                                                        if (
                                                            _.isArray(syncedActivityModels) &&
                                                            syncedActivityModels.length === 1
                                                        ) {
                                                            // One activity found
                                                            ...
                                                            _.forEach(
                                                                ...
                                                                (activityModel: SyncedActivityModel) => {
                                                            ...
                                                            const elevateSportResult = this.convertToElevateSport(
                                                                sportsLibActivity
                                                            );
    ...
    public extractActivityStreams(sportsLibActivity: ActivityInterface): ActivityStreamsModel {
        ...
        try {
            if (
                ActivityTypesHelper.getActivityGroupForActivityType(sportsLibActivity.type) ===
    ...
    public deflateActivitiesFromArchive(archiveFilePath: string, deleteArchive: boolean = false): Promise<string[]> {
        return new Promise((resolve, reject) => {
            ...
            if (Service.instance().isPackaged && !Service.instance().isLinux()) {
                options.unar =
                    Service.instance().getResourceFolder() + "/app.asar.unpacked/node_modules/all-unpacker/unar";
            }
                    ...
                    try {
                        ...
                        extractedActivityFiles.forEach(extractedActivityFile => {
                            ...
                            const newActivityPath =
                                ...
                                (relativeExtractedDirName
                                    ? "-" + BaseConnector.hashData(relativeExtractedDirName, 6)
                                    : "") +
                                "-" +
                                extractedFileName;
            ...
            .reduce((previousPromise: Promise<void>, file: string) => {
                return previousPromise.then(() => {
                    ...
                    if (!isDirectory) {
                        ...
                        const fileExtension = path.extname(file).slice(1);
                        const isArchiveFile =
                            ["zip", "rar", "gz", "tar", "7z", "bz2", "zipx", "xz"].indexOf(fileExtension) !== -1;
                        if (isArchiveFile) {
