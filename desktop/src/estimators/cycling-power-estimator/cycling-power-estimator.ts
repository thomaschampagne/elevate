...
export namespace CyclingPower {
	...
	export class Estimator {
		 ...
		 * @param params
		 */
		public static csvPowerAnalysisData(csvOutputPath: string = __dirname + "/power_analysis_data.csv", endSpeed: number = 50, maxGrade: number = 25, params: Partial<CyclingPower.Params> = Estimator.DEFAULT_PARAMS): void {
		...
		private static estimateForces(velocity, params) {
			...
			// Calculate FGravity
			const FGravity = 9.8067 *
				(params.riderWeightKg + params.bikeWeightKg) *
				Math.sin(Math.atan(params.gradePercentage / 100.0));
