...
RemoteLinksModifier.prototype = {
    ...
    modifyActivityPage_: function() {
        ...
        var remoteViewActivityLinksArray = [
            ["VV Activity shorcut", 'http://veloviewer.com/activities/', '?referrer=stravistiX', '', 'remoteLink/vv_act'],
            ["VV Seg. Compare", '#', '', 'onclick="javascript:stravistiX.remoteLinksModifier.veloviewerSegmentCompare(window.event)"', 'remoteLink/vv_seg_comp'],
            ["Surface", 'http://strava-tools.raceshape.com/erea/?url=', '', '', 'remoteLink/surface']
        ];

        // Activity page
        // Adding remote view links on left panel
        var htmlRemoteViewForActivity = "<li class='group'>";
        htmlRemoteViewForActivity += "<div class='title' id='stravistix_remote_title' style='font-size: 14px; cursor: pointer;' mssg_id='remoteLink/section'>Remote Views</div>";
        ...
        $.each(remoteViewActivityLinksArray, function() {
            ...
            htmlRemoteViewForActivity += "<a data-menu='' " + this[3] + " target='_blank' style='color: #333;' href='" + this[1] + pageView.activity().id + this[2] + "' mssg_id='" + this[4] + "'>" + this[0] + "</a>";
        });
        ...
        if (this.authorOfActivity) {
            var htmlForTCXExport = "<li><a href='" + window.location.pathname + "/export_tcx' mssg_id='remoteLink/export_tcx'>Export TCX</a></li>";
    ...
    modifySegmentPage_: function() {
        ...
        var remoteViewSegmentLinksArray = [
            ["<img width='24px' style='vertical-align:middle' src='" + this.appResources_.veloviewerIcon + "'/> <span mssg_id='remoteLink/vviewer'>VeloViewer</span>", 'http://veloviewer.com/segment/', '?referrer=stravistiX'],
            ["<img width='24px' style='vertical-align:middle' src='" + this.appResources_.pollIcon + "'/> <span mssg_id='remoteLink/seg_stats'>Segment Stats</span>", 'http://www.jonathanokeeffe.com/strava/segmentDetails.php?segmentId=', '']
        ];
        ...
        html += "<div class='selection'><img style='vertical-align:middle' src='" + this.appResources_.remoteViewIcon + "'/> <span mssg_id='remoteLink/rem_seg_view'>Remote Segment View</span></div>";
