...
export class AthleteStatsModifier extends AbstractModifier {
	...
	protected init(activities: any): void {
		...
		this.progressThisYear.find("#elevate_yearProgress_incVirtualRides").prop("checked", Cookies.get("elevate_yearProgress_incVirtualRides") === "true");
		this.progressThisYear.find("#elevate_yearProgress_incVirtualRides").on("click", () => {
			Cookies.set("elevate_yearProgress_incVirtualRides", $("#elevate_yearProgress_incVirtualRides").prop("checked"), {expires: 365});
			this.handleProgressStatsForceRefresh();
		});

		this.progressThisYear.find("#elevate_yearProgress_incCommutes").prop("checked", Cookies.get("elevate_yearProgress_incCommutes") === "true");
		this.progressThisYear.find("#elevate_yearProgress_incCommutes").on("click", () => {
			Cookies.set("elevate_yearProgress_incCommutes", $("#elevate_yearProgress_incCommutes").prop("checked"), {expires: 365});
			this.handleProgressStatsForceRefresh();
		});
	}
	...
	protected formatData(activities: any[]): any[] {
		...
		const includeVirtualRide: boolean = (Cookies.get("elevate_yearProgress_incVirtualRides") === "true");

		const includeCommutes: boolean = (Cookies.get("elevate_yearProgress_incCommutes") === "true");
	...
	public modify(): void {
		...
		let info = "<h4>⚠ Current year progressions will be disabled once 'Distance last 30d & year' features have been migrated to the <a onclick='window.open(\"" + this.appResources.settingsLink + "#/yearProgressions\", \"_blank\");'>Elevate App</a> itself. ";
		info += "Actually this feature being developed in a more advanced way into Elevate App. <a onclick='window.open(\"https://github.com/thomaschampagne/elevate/issues/760#issuecomment-462072783\", \"_blank\");'>Current status, screenshots and brainstorming is available here. Feel free to participate.</a></h4>";

		this.progressThisYear = $("<div class='section'><h3>My year progressions to current month/day (old version) <span id='athleteStatsLoading' class='ajax-loading-image'></span></h3>" + info + "<div>This panel displays your progress for each beginning of year to current month and day. Assuming today is May 25, this panel shows \"What I've accomplished by May 25 of this year compared to previous years during the same period.\"<br/><br/><input type='checkbox' id='elevate_yearProgress_incVirtualRides'/> Include Virtual Rides <input type='checkbox' id='elevate_yearProgress_incCommutes'/> Include Commutes</div><div><ul class='switches'><li><a class='button btn-xs' data-activity-type='0' style='display: none;'>Cycling</a></li><li><a class='button btn-xs' data-activity-type='1' style='display: none;'>Running</a></li><li class='last-child' id='athleteStatsShowChart' style='display: none;'><a class='button btn-xs' style='max-height: 24px;' title='Chart'><img style='height: 12px;' src='" + self.appResources.trendingUpIcon + "'/></a></li><li>&nbsp;&nbsp;&nbsp;<a href='#' id='athleteStatsLoadingForceRefresh' style='display: none'>Force refresh</a></li></ul></div></div>");
