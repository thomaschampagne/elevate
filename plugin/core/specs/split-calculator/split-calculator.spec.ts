...
import { SplitCalculator } from "../../scripts/processors/split-calculator";
...
describe("SplitCalculator", () => {
	...
	it("should get monotonically decreasing rider power curve of activity 1480020375", (done: Function) => {
		...
		const secondsUpTo1Min = Array.from(Array(60 + 1).keys()).slice(1);
		const minsUpTo20Mins = Array.from(Array(20 + 1).keys()).slice(2);
		const scaleRanges: number[] = [...[...secondsUpTo1Min,
			...minsUpTo20Mins.map(m => m * 60)].filter(s => s < maxTime),
			maxTime];

		// When
		const splitCalculator = new SplitCalculator(timeScale, wattsData);
		const results = splitCalculator.getBestSplitRangesMonotonicDecrease(scaleRanges);

		// Then

		// Ensure specific powers are as expected (as getBestSplitRanges)
		expect(Math.floor(results.find(r => r.range === 60).result)).toEqual(expectedPowerOneMinute);
		expect(Math.floor(results.find(r => r.range === 20 * 60).result)).toEqual(expectedFTP);
	...
	it("Greedy algo power curve should use larger scale best when smaller scale is lower", (done: Function) => {

		// Given
		// An example where the average over 3 seconds is 600W
		// But the default algorithm will only find a best average of 450W for 2 seconds
		const timeScale: number[] = [0, 1, 2];
		const wattsData: number[] = [900, 0, 900];

		// When
		const splitCalculator = new SplitCalculator(timeScale, wattsData);
		const defaultResults = splitCalculator.getBestSplitRanges([2]);
		const greedyAlgoResults = splitCalculator.getBestSplitRangesGreedy([2]);

		// Then
		expect(defaultResults[0].range).toEqual(2);
		expect(defaultResults[0].result).toEqual(450);

		expect(greedyAlgoResults[0].range).toEqual(2);
		expect(greedyAlgoResults[0].result).toEqual(600);
