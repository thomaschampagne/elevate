...
describe('YearProgressTableComponent', () => {
	...
	it("should provide year progressions distance rows for data table display", () => {
		...
		const progressionAtDayModels: ProgressionAtDayModel[] = yearProgressService.findProgressionsAtDay(component.yearProgressModels,
			dayMoment, component.selectedProgressType.type, component.selectedYears);
		...
		expect(progressionRow2018.date).toEqual("March 01, 2018");
		...
		expect(progressionRow2018.deltaSign).toEqual(DeltaSign.NEGATIVE);
		expect(progressionRow2018.deltaSignSymbol).toEqual(YearProgressTableComponent.DELTA_SIGN_NEGATIVE);
		expect(progressionRow2018.deltaClass).toEqual(DeltaSign.NEGATIVE.toString());

		const progressionRow2017 = progressionRows[1];
		expect(progressionRow2017.date).toEqual("March 01, 2017");
		...
		expect(progressionRow2017.deltaSign).toEqual(DeltaSign.NEGATIVE);
		expect(progressionRow2017.deltaSignSymbol).toEqual(YearProgressTableComponent.DELTA_SIGN_NEGATIVE);
		expect(progressionRow2017.deltaClass).toEqual(DeltaSign.NEGATIVE.toString());

		const progressionRow2016 = progressionRows[2];
		expect(progressionRow2016.date).toEqual("March 01, 2016");
		...
		expect(progressionRow2016.deltaSign).toEqual(DeltaSign.POSITIVE);
		expect(progressionRow2016.deltaSignSymbol).toEqual(YearProgressTableComponent.DELTA_SIGN_POSITIVE);
		expect(progressionRow2016.deltaClass).toEqual(DeltaSign.POSITIVE.toString());

		const progressionRow2015 = progressionRows[3];
		expect(progressionRow2015.date).toEqual("March 01, 2015");
		...
		expect(progressionRow2015.deltaSign).toEqual(DeltaSign.NAN);
		expect(progressionRow2015.deltaSignSymbol).toEqual(null);
		expect(progressionRow2015.deltaClass).toEqual(DeltaSign.NAN.toString());
	...
	it("should provide proper year progressions time rows between two years (1)", () => {
		...
		expect(firstRow.date).toEqual("August 15, 2017");
		...
		expect(firstRow.deltaSign).toEqual(DeltaSign.NEGATIVE);
		expect(firstRow.deltaSignSymbol).toEqual(YearProgressTableComponent.DELTA_SIGN_NEGATIVE);
		expect(firstRow.deltaClass).toEqual(DeltaSign.NEGATIVE.toString());

		const secondRow = progressionRows[1];
		expect(secondRow.date).toEqual("August 15, 2016");
		...
		expect(secondRow.deltaSign).toEqual(DeltaSign.NAN);
		expect(secondRow.deltaSignSymbol).toEqual(null);
		expect(secondRow.deltaClass).toEqual(DeltaSign.NAN.toString());
	...
	it("should provide proper year progressions time rows when no delta between two years (2)", () => {
		...
		expect(firstRow.deltaSign).toEqual(DeltaSign.UNSIGNED);
		expect(firstRow.deltaSignSymbol).toEqual(YearProgressTableComponent.DELTA_SIGN_UNSIGNED);
		expect(firstRow.deltaClass).toEqual(DeltaSign.UNSIGNED.toString());
	...
	it("should provide proper year progressions time rows when no delta between two years (3)", () => {
		...
		expect(firstRow.deltaSign).toEqual(DeltaSign.POSITIVE);
		expect(firstRow.deltaSignSymbol).toEqual(YearProgressTableComponent.DELTA_SIGN_POSITIVE);
		expect(firstRow.deltaClass).toEqual(DeltaSign.POSITIVE.toString());
